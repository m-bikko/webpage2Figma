...
const FIXTURES = [
  ...
  'blur',
  'image-fit', 'image-bg', 'image-data', 'image-cors', 'image-broken', 'video',
] as const
...
test('стекинг: порядок отрисовки не совпадает с порядком DOM', async ({ page }) => {
  ...
  // .under (z-index: -1) красится РАНЬШЕ потокового .flow — так его
  // рисует Chrome, и теперь так же считает резолвер.
  //
  // Утверждение здесь появилось не сразу. Пока `position: relative` с
  // `z-index: auto` считался атомарной единицей, потомки из него не
  // поднимались, и .under получал индекс 3 вместо 1. Тогда сказать
  // было нечего: «раньше» упало бы, «позже» заморозило бы неверное
  // поведение, — и тест намеренно молчал о порядке, ограничиваясь
  // «узел не потерялся». Молчание было честным, но проверкой не было.
  //
  // Упрощение снято: всплывшие узлы переносятся к предку-контексту
  // (packages/serializer/src/hoist.ts), и порядок стал верным.
  const under = at(170, 140, 140, 140)
  const flow = flat.find((node) => node.h === 40)
  expect(under).toBeDefined()
  expect(flow, 'потоковая полоса .flow высотой 40 обязана быть в дереве')
    .toBeDefined()
  expect(under.order).toBeLessThan(flow?.order ?? -1)
})
