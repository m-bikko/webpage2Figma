...
/** Сгенерированное содержимое обязано быть НАЗВАНО отдельно.
 *
 *  Проверка появилась после разбора: обязательный код
 *  `deferred.pseudo-element` на фикстуре `pseudo-element-flow` давал
 *  потоковый `.prefix`, и тест зеленел, ничего не говоря про счётчики.
 *  А счётчики в тот момент исчезали МОЛЧА: `counter(n)` не разбирался
 *  в литерал, видимого текста не давал, краски у `li::before` нет — и
 *  разбор возвращал «переносить нечего». Номера строк в блоках кода,
 *  самая частая форма сгенерированного содержимого на живых
 *  страницах, пропадали без единой записи.
 *
 *  Утверждается поэтому не код, а СМЫСЛ сообщения: оно обязано
 *  упоминать счётчик, иначе пользователь не узнает, что именно
 *  потеряно. */
test('счётчик в псевдоэлементе назван, а не проглочен', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.goto(fixtureUrl('pseudo-element-flow'))
  const { report } = await captureScreen(page, 's0', 'Desktop')

  const generated = report.filter(
    (item) => item.code === 'deferred.pseudo-element'
      && item.message.includes('counter('),
  )
  expect(
    generated.length,
    `отчёт обязан назвать сгенерированное содержимое, а в нём: ` +
