...
/** Участвуют ПЯТЬ фикстур из двенадцати, и это не недосмотр.
 *
 ...
 *  рендер верен и проверяется наравне с остальными.
 *
 *  `transformed/` и `radial-gradient/` обязаны расходиться: трансформы не
 *  реализованы, а радиальному градиенту в SVG нет соответствия вовсе,
 *  поэтому рендер заведомо неверен и подогнанный под него порог был бы
 *  ложью в чеклисте. Они проверяются `diagnostics.spec.ts` — там
 *  утверждается НАЛИЧИЕ кодов, объясняющих пропуск.
 *
...
const FIXTURES = ['boxes', 'stacking', 'flex', 'text', 'gradient'] as const
