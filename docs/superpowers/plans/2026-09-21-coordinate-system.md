...
**Что произойдёт с e2e после этой задачи.** Ничего: 171 тест остаётся зелёным, и это не признак того, что задача не сработала.

Причин три, и все три стоит понимать, потому что они объясняют, почему разрыв между контрактом и сериализатором не виден автоматически. Первая: собранный IIFE-бандл не пересобирается в этой задаче, поэтому в страницу впрыскивается прежний код сериализатора. Вторая: `bundle.spec.ts` строит версию бандла из свежего импорта `@h2d/ir`, а не из того, что вернул сериализатор, — рассогласования версий там не возникает по построению. Третья: ни одна нынешняя фикстура не имеет вложенности, превышающей щедрый порог в пятьдесят крат.

Из этого следует практический вывод для Task 2: **проверка бандла не сравнивает версию, объявленную сериализатором, с версией, которую ожидает валидатор.** Стоит подумать, не нужен ли такой тест — но не здесь.
...
Затем опустить до `1` и прогнать снова: обязан упасть **второй** тест — тот, где ребёнок законно вынесен наружу. Это доказывает, что порог не задушил легитимный случай, а не только что он ловит нелегитимный. Вернуть.

Множитель именно `1`, а не `2`, и арифметика здесь важна: в фикстуре второго теста родитель 100×100, а смещение ребёнка 150. При множителе 2 порог равен 200, и 150 его не превышает — тест остаётся зелёным, то есть доказательство не выполняется. Первая редакция этого шага говорила `2` и была неверна.
...
```ts
  ...
  /** Обратная к накопленной. Хранится, а не считается на каждом узле:
   *  обращение матрицы на дереве из десятков тысяч элементов — заметная
   *  работа, а значение меняется только у трансформированных узлов. */
...
```ts
  ...
  /** Положение в системе родителя. Формула НЕ сводится к вычитанию двух
   *  экранных положений, и это выяснилось измерением против эталона —
   *  той же раскладки с `transform: none`, прочитанной из DOM.
   *
   *  Вычитание неверно дважды.
   *
   *  Первое: разность двух экранных положений — вектор в ЭКРАННЫХ осях.
   *  Под поворотом предка на 20° ребёнок с локальными (20, 0) даёт
   *  (18.79, 6.84), то есть ровно `R(20°)·(20,0)`. Положив это в `rect`,
   *  мы повернули бы ребёнка дважды: рендерер повернёт группу ещё раз.
   *  Поэтому разность домножается на ОБРАТНУЮ матрицу предка.
   *
   *  Второе: `originUnderMatrix` возвращает локальный ноль ПОСЛЕ
   *  собственной трансформы узла. Для позиционирования детей это верно,
   *  для собственного `rect` — нет, потому что `transform` едет
   *  отдельным полем и рендерер применит его к группе. Поэтому
   *  собственный перенос вычитается.
   *
   *  Следствие, которое легко упустить: в первой редакции этого шага
   *  `matrixAboutOrigin` была ЦЕРЕМОНИЕЙ — она меняет только перенос, а
   *  `originUnderMatrix` читает лишь линейную часть. Несущей она
   *  становится ровно здесь, когда перенос начинают вычитать. */
  const relative = localOffset({
    screenOrigin,
    parentOrigin: ctx.parentOrigin,
    ancestorInverse: ctx.ancestorInverse,
    ownMatrix,
  })
  const box = { x: relative.x, y: relative.y, w: size.w, h: size.h }
...
## Task 2b: Боксы строк в системе своего узла

Найдено исполнителем Task 2 и подтверждено измерением: `LineBox` нёс **абсолютные** координаты. Глубоко вложенный абзац с локальным `rect` (50, 50) давал бокс строки (350, 253).

Сегодня это ещё согласовано, потому что рендерер тоже читает всё как абсолютное. Но Task 3 научит его складывать смещения по дереву — и тогда фигуры станут позиционироваться накоплением, а текст нет. Внутри трансформированной группы текст преобразовался бы **дважды**: один раз группой, второй — собственными абсолютными координатами.

Исполнитель Task 2 не мог это исправить: нужен `packages/ir`, а граница задачи его запрещала. Это тот случай, когда граница сработала правильно — он сообщил вместо того, чтобы расширить объём.

- [ ] **Step 1: Док-комментарий `LineBox` в `packages/ir/src/types.ts`**

Координаты относительно `rect` своего узла. В комментарии записать, почему абсолютные были незаметно неверны и чем это стало бы после Task 3.

- [ ] **Step 2: `readText` и `readLines` принимают начало узла вместо прокрутки**

Подпись меняется с `(el, cs, scrollX, scrollY)` на `(el, cs, origin)`, где `origin` — экранное положение левого верхнего угла бокса узла. Внутри `readLines` вместо прибавления прокрутки вычитается `origin`.

Прокрутка отсюда уходит не случайно: она входит в положение корня и наследуется вложенностью, ровно как для `rect`.

- [ ] **Step 3: Вызов в обходчике**

`readText(el, cs, screenOrigin)` — та же величина, что уже вычислена для `rect`.

- [ ] **Step 4: Проверить на вложенном случае**

Собрать страницу, где абзац лежит внутри двух смещённых контейнеров, снять `pnpm capture` и убедиться: бокс первой строки близок к нулю по `x`, а не равен экранному положению. На неглубокой странице проверка бессмысленна — там локальное и абсолютное совпадают, и ошибка невидима.

- [ ] **Step 5: Коммит**

```bash
git add packages/ir packages/serializer
git commit -m "fix: боксы строк в системе координат своего узла"
```
...
**Боксы строк тоже стали локальными** — относительно `rect` своего узла, задачей 2b. Значит `renderTextLines` обязана складывать их координаты с абсолютным положением узла, а не использовать как есть:

```ts
    const lineX = absolute.x + line.x
    const lineY = absolute.y + line.y
```

Без этого текст остался бы там, где был до перехода на локальные координаты, а внутри трансформированной группы преобразовался бы дважды. Добавь тест: текстовый узел внутри смещённого родителя даёт строку в сумме смещений.
