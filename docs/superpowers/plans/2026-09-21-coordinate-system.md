...
Отмечено по существу, а не по галочкам: галочки в шагах не проставлялись, потому что
задачи исполнялись подагентами и ставить их задним числом значило бы утверждать
непроверенное. Проверено следующее.

| что обещано | чем подтверждено |
|---|---|
| `rect` в координатах родителя | `IR_VERSION = 2`, инвариант `checkLocalCoordinates`, все снапшоты IR перегенерированы |
| групповые эффекты на поддереве | `group-effects`, `transform-nested`, `blend-isolated` в pixel-diff, 0 px на всех пяти ширинах |
| три диагностики сняты | `blendIsolation`, `blurDescendant`, `opacityGroup` больше не порождаются |
| `transformDescendant` остался осознанно | порождается только под скосом и трёхмерной матрицей — задокументированный остаток |
| проверки не пусты | плоская отрисовка роняет 5 ширин; изоляция по прежнему условию роняет 5 ширин |

Сверх плана: исправлен дефект изоляции, которого план не предвидел, —
разбор в [[group-effects]] и случай 11 в [[correctness-strategy]].
...
Причин три, и все три стоит понимать, потому что они объясняют, почему разрыв между контрактом и сериализатором не виден автоматически. Первая: собранный IIFE-бандл не пересобирается в этой задаче, поэтому в страницу впрыскивается прежний код сериализатора. Вторая: `bundle.spec.ts` строит версию бандла из свежего импорта `@w2f/ir`, а не из того, что вернул сериализатор, — рассогласования версий там не возникает по построению. Третья: ни одна нынешняя фикстура не имеет вложенности, превышающей щедрый порог в пятьдесят крат.
...
Expected: PASS. Тестов в `@w2f/ir` станет на три больше.
