...
export const resolveAssets = async (
  ...
  for (const request of requests) {
     ...
     *
     *  Растеризовать его было бы проще, но вектор в Figma лучше растра
     *  ровно тем, ради чего дизайнер открывает макет: его можно
     *  масштабировать и править. Эталонный рендерер рисует такие байты
     *  через `data:`-URL в `<image>` — браузер это умеет, — а плагин
     *  строит из них векторный узел.
     *
     *  Ужатие и переупаковка ниже к вектору неприменимы: у него нет
