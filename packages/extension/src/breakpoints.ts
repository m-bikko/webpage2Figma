...
/** Проходит страницу прокруткой, чтобы ленивое содержимое загрузилось.
 *
 *  Зачем. `loading="lazy"` и `IntersectionObserver` начинают загрузку
 *  только когда элемент приближается к видимой области. Захват без
 *  прокрутки застаёт всё, что ниже сгиба, незагруженным, и такие
 *  картинки честно становятся заглушками — честно, но бесполезно: на
 *  длинной странице теряется большинство изображений, ровно тех, ради
 *  которых макет и снимают.
 *
 *  Измерено на восьми живых страницах: 73 записи «источник не
 *  загружен», семь страниц из восьми. Причина не CORS — его воркер
 *  обходит, — а именно то, что загрузка не начиналась.
 *
 *  Прокрутка ВОЗВРАЩАЕТСЯ в исходное положение. Расширение обещает
 *  снять страницу такой, какой её оставил пользователь, и уехавший
 *  скролл сломал бы это обещание вдвойне: сместились бы и липкие
 *  элементы, положение которых снимается как есть.
 *
 *  Побочное действие признаётся прямо: страницы с анимациями «появись
 *  при прокрутке» после этого прохода показывают проявленное
 *  содержимое. Это ближе к тому, что человек на странице видел, чем
 *  пустые места, — но это ИЗМЕНЕНИЕ состояния, и потому оно описано
 *  здесь, а не спрятано.
 *
 *  Шаг равен видимой высоте: мельче — дольше без выигрыша, крупнее —
 *  наблюдатель может не успеть сработать на пропущенном участке. */
export const primeLazyContent = async (tabId: number): Promise<number> => {
  const passes = await chrome.scripting.executeScript({
    target: { tabId },
    world: 'MAIN',
    func: async () => {
      ...
      let steps = 0
      /** Предел на число шагов: бесконечная лента подгружает
       *  содержимое по мере прокрутки, и без предела проход не
       *  кончился бы никогда — захват выглядел бы повисшим. */
      for (let y = 0; y < full() && steps < 40; y += step) {
        window.scrollTo(0, y)
        steps += 1
        /** Пауза нужна: наблюдатель пересечения срабатывает после
         *  кадра, а не в момент присваивания scrollTo. */
