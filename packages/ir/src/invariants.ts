...
/** ОГРАНИЧЕНО НЕРЕАЛИЗОВАННЫМИ ФИЧАМИ. `blend`, `blur` и `kind: 'vector'`
 ...
 *
 *  `transform` уже УБРАН — трансформы переносятся и проверяются pixel-diff,
 *  поэтому диагностика для них больше не обязательна и требовать её значило
 *  бы отвергать корректные бандлы. Это то самое пофичное удаление, которое
 *  предписывает абзац выше; ссылка на него оставлена как образец. */
