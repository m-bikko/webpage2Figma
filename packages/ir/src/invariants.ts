...
/** ОГРАНИЧЕНО НЕРЕАЛИЗОВАННЫМИ ФИЧАМИ. ФОНОВОЕ размытие и `kind: 'vector'`
 *  признаны в контракте IR, но их построение откладывается (см. коды
 *  `deferred.*` в `codes.ts`). До тех пор любой узел, несущий одну из этих
 *  фич, ОБЯЗАН сопровождаться парной диагностикой — иначе фича молча
 *  теряется, что и обнаружил ревьюер на повёрнутом блоке с пустым report.
 *
 *  Этот блок — временный костыль ровно под этот пробел и должен удаляться
 *  ПОФИЧНО по мере реализации построения. Без удаления он окаменеет:
 *  корректный бандл без диагностики (потому что больше нечего откладывать)
 *  начал бы отвергаться этой же проверкой. Каждую строку `deferred.push(...)`
 *  убирать вместе с фичей, а не всю функцию сразу.
 *
 *  `transform`, `blend` и размытие СЛОЯ уже УБРАНЫ — все три переносятся
 *  и проверяются pixel-diff, поэтому диагностика для них больше не
 *  обязательна и требовать её значило бы отвергать корректные бандлы. Это
 *  то самое пофичное удаление, которое предписывает абзац выше; ссылка на
 *  него оставлена как образец.
 *
 *  У размытия слоя оговорка ТОЧНО ТОГО ЖЕ РОДА, что у двух предыдущих
 *  фич, и её надо читать как незакрытый пробел, а не как решённый вопрос.
 *  В CSS `filter: blur()` размывает элемент ВМЕСТЕ С ПОДДЕРЕВОМ, а
 *  рендерер плоский: он вешает `<filter>` на сам узел, а его потомки
 *  рисуются отдельными фигурами и приезжают РЕЗКИМИ. Пока размытие было
 *  отложено целиком, узел-родитель нёс `deferred.blur`, и это заодно
 *  объясняло, что не размыто и поддерево. Теперь родитель переносится
 *  верно, объяснение исчезло вместе с диагностикой, а резкость потомков
 *  осталась. Измерено зондом: блок `filter: blur(6px)` с вложенным
 *  блоком даёт 2362 расходящихся пикселя при ПУСТОМ отчёте. Фикстура
 *  `blur/` этот случай не проверяет намеренно — в ней у размытых блоков
 *  нет содержимого, иначе измерялось бы сразу два эффекта. Пробел пока НЕ
 *  закрыт: закрывать его так же, как закрыты Task 5b и Task 6b —
 *  отдельным кодом `fidelity.blur-descendant` и его распространением
 ...
 *  наложением под изолирующим предком отрисуется неверно — против всего
 *  холста вместо своей изолированной группы — и это не продиагностировано
 *  никак. Фикстура `blend/` этот случай не проверяет: в ней нет
 *  `isolation: isolate`. Оставлено как известный пробел, а не тихая
 *  ошибка — по тому же правилу, что породило `fidelity.transform-descendant`
 *  в Task 5b. */
/** Отложенные фичи обязаны быть продиагностированы.
 *
 *  Спека §7.1b требует: пока фича не реализована, встреча с ней порождает
 *  Diagnostic. До этой проверки правило было записано в трёх документах и
 *  не проверялось нигде: единственной защитой были шесть рукописных фикстур,
 *  а на живой странице повёрнутый блок приезжал осепараллельной коробкой
 *  при пустом отчёте.
 *
 *  ВНИМАНИЕ: блок удаляется ПО ФИЧЕ по мере их реализации. Реализовали
 *  transform — убрали проверку transform (это уже сделано), и так далее.
 *  Если этого не делать, блок превратится в окаменелость и начнёт
 *  отвергать КОРРЕКТНЫЕ бандлы, в которых фича работает и диагностировать
 *  её больше не нужно. */
const checkDeferredDiagnosed = (bundle: Bundle): InvariantError[] => {
  ...
  for (const [screenIndex, screen] of bundle.screens.entries()) {
    for (const node of allNodes(screen)) {
      ...
      /** Требование сузилось до ФОНОВОГО размытия: `filter: blur()`
       *  переносится и проверяется pixel-diff на фикстуре `blur/`
       *  (измерено 0 расходящихся пикселей), поэтому требовать для него
       *  диагностику значило бы отвергать корректные бандлы. У
       *  `backdrop-filter` переносить нечего: рендерер плющит дерево, и
       *  «того, что за элементом» у него нет. */
