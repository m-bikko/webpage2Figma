 ...
 *
 *  ИМЕНА РЕЖИМОВ — ИЗ FIGMA, А НЕ ИЗ CSS, и это ловушка. `fill` здесь
 *  означает «заполнить бокс, сохранив пропорции, лишнее обрезать», то
 *  есть CSS `object-fit: cover`. А CSS `object-fit: fill` (растянуть,
 *  пропорции не сохранять) режимом Figma не выражается вовсе и приезжает
 *  как `crop` с РАЗНЫМИ `scaleX` и `scaleY`. Слово одно, смысл разный.
 *
 *  Картинку определяют `scaleX`/`scaleY` и смещения, а НЕ `mode`:
 *  референс-рендерер читает именно их. Поэтому `mode` гейтом не
 *  проверяется — его единственная проверка сейчас это таблица в
 *  юнит-тесте, а настоящая сверка приходит с плагином. */
...
/** `grid` — отдельный член, а не сведение к колонке.
 *
 *  Figma двумерного auto-layout не имеет, и соблазн записать сетку
 *  колонкой велик. Но IR описывает СТРАНИЦУ, а не то, во что её
 *  удобно превратить: сведение здесь означает, что плагин уже не
 *  узнает, была ли это сетка, и будет предсказывать раскладку по
 *  неверной оси.
 *
 *  Измерено на живой странице: сведение сетки к колонке было главной
 *  причиной отказов от auto-layout — двухколоночная сетка
 *  предсказывалась как стопка, и предсказание расходилось с
 *  измеренным на сотни пикселей. Решение, как выразить сетку, теперь
 *  принимает плагин: одномерную он раскладывает по её настоящей оси,
 *  двумерную честно отвергает. */
...
export type SelfLayout = {
  ...
  grow: number
  shrink: number
  /** Внешние отступы. Figma их не выражает — у auto-layout есть
   *  только отступы контейнера и один зазор на всех, — но без них
   *  НЕЛЬЗЯ ПРЕДСКАЗАТЬ раскладку: на живой странице отступы детей
   *  давали расхождение в сотни пикселей, и auto-layout отвергался
   *  там, где раскладка совершенно обычная.
   *
   *  Поле нужно не для переноса, а для объяснения. Одинаковые отступы
   *  сворачиваются в отступы контейнера и зазор; разные честно
   *  называются причиной отказа. */
  margin: Sides
  /** `margin: auto` по сторонам. Отдельно от чисел, потому что
   *  `auto` — не величина, а правило: браузер отдаёт его уже
   *  вычисленным в пикселях, и отличить «поставили 16» от
   *  «получилось 16 само» иначе нельзя. Центрирование блока через
   *  `margin: 0 auto` — самая частая раскладка, которую флекс сам по
   *  себе не объясняет. */
  marginAuto: { horizontal: boolean; vertical: boolean }
}
 ...
 *
 *  Координаты ОТНОСИТЕЛЬНО `rect` своего узла — в той же системе, в
 *  которой дети выражены относительно родителя.
 *
 *  До плана 3 они были абсолютными, и это оставалось незаметным, пока
 *  рендерер тоже читал всё как абсолютное. Как только он начал складывать
 *  смещения по дереву, расхождение стало бы дефектом: фигуры
 *  позиционировались бы накоплением, а текст нет. Внутри трансформированной
 *  группы текст преобразовался бы ДВАЖДЫ — один раз группой, второй раз
 *  собственными абсолютными координатами. */
