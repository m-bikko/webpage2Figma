 ...
 *
 *  Конический градиент отдельным членом НЕ появился и появится не
 *  раньше, чем у гейта возникнет способ его проверить: в SVG
 *  конического градиента нет, а значит эталонный рендерер нарисовать
 *  его не может, и сверять результат было бы не с чем. */
...
/** Радиальный градиент, заданный центром и радиусами.
 *
 *  Координаты и радиусы НОРМАЛИЗОВАНЫ по боксу узла, как и у
 *  линейного: центр — доля от ширины и высоты, радиусы — тоже. Так
 *  величины не зависят от размера бокса, и обе стороны — SVG и Figma —
 *  восстанавливают их одинаково.
 *
 *  Радиусы ДВА, а не один, потому что CSS по умолчанию рисует эллипс:
 *  `radial-gradient(...)` без слова `circle` растягивает градиент по
 *  сторонам бокса. Свести эллипс к кругу значило бы испортить
 *  подавляющее большинство случаев — именно форма по умолчанию
...
export type Screen = {
  ...
  /** Цвет ХОЛСТА — того, что браузер красит под всем содержимым.
   *
   *  По CSS холст — отдельная от `body` сущность: фон корневого
   *  элемента распространяется на весь холст, а если корень фона не
   *  задаёт — на холст уходит фон `body`, и красится им весь вьюпорт,
   *  а не бокс `body`. Так короткая тёмная страница остаётся тёмной до
   *  самого низа окна. Не задан ни один — браузер красит холст сам,
   *  и его цвет ни один API не отдаёт: он выведен из измерений
   *  (см. serializer `page-canvas.ts`).
   *
   *  Хранить это заливкой корня нельзя: корень — бокс `body`, а холст
   *  покрывает весь экран, включая то, куда `body` не дотягивается.
   *  Первая редакция так и делала, и pixel-diff на фикстуре
   *  `canvas-bg` показал разницу в миллион пикселей. */
  canvas: Rgba8
  root: IrNode
