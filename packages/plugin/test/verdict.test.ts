...
describe('autoLayoutVerdict: исключения по построению', () => {
  ...
  /** `flex-grow` распределяет свободное место по своим правилам, и
   *  размеры детей в IR уже включают результат. Повторить это флексом
   *  Figma можно не всегда. */
  it('растягивающийся ребёнок — небезопасно', () => {
    const parent = rowParent([
      { x: 0, y: 0, w: 50, h: 20 },
      { x: 60, y: 0, w: 40, h: 20 },
    ])
