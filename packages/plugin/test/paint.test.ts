...
describe('gradientPaint: то, что проверяемо безусловно', () => {
  ...
  /** Альфа остановки живёт ВНУТРИ ЦВЕТА — в отличие от сплошной
   *  краски, где она отдельным полем `opacity`. Формы разные, и это
   *  не наша прихоть: `ColorStop.color` в Figma объявлен как `RGBA`,
