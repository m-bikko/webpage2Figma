...
describe('imageNodeFor: неравномерное растяжение', () => {
  ...
  /** Отметки «требует сверки» здесь БОЛЬШЕ НЕТ: вопрос закрыт замером
   *  в настоящей Figma. `CROP` без заданного `imageTransform`
   ...
   *  у `cover` — [45, 2, 64] (левые ~17% обрезаны). Браузер даёт
   *  [3, 3] и [45, 3]. Тест перевёрнут, а не удалён: он теперь
   *  сторожит, чтобы отметка не вернулась и не стала шумом. */
  it('отметки «требует сверки» не получают: проверено замером', () => {
    const node = imageNodeFor('n1', rect, place({ scaleX: 1.875, scaleY: 2.8125 }), 'a0', natural)
  ...
  it('равномерный масштаб отметки тоже не получает', () => {
    const node = imageNodeFor('n1', rect, place({ scaleX: 2, scaleY: 2 }), 'a0', natural)
    expect(node.needsVerification).toEqual([])
  })
