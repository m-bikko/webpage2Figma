...
export const autoLayoutVerdict = (node: IrNode): AutoLayoutVerdict => {
  ...
  /** СОБСТВЕННЫЙ ТЕКСТ контейнера — тоже элемент раскладки.
   *
   *  `<h1 class="flex">Rapidly <span>build</span> modern…</h1>`: в CSS
   *  каждый кусок текста между элементами становится анонимным
   *  флекс-элементом и занимает место, а решатель видит только
   *  элементы-детей. Отказ «не объясняется флексом» с числами тут
   *  вводил в заблуждение — раскладка объясняется, просто не теми
   *  детьми. В Figma такое не выразить: текст контейнера едет одним
   *  узлом и с детьми перемежаться не может. Найдено пробой на живой
   *  странице: заголовки tailwindcss.com. */
  if (node.kind === 'text' && node.text.lines.length > 0) {
    return {
      safe: false,
      reason: 'у контейнера есть собственный текст между детьми: в CSS он '
        + 'занимает место как анонимный флекс-элемент, а в Figma один '
        + 'текстовый узел с детьми перемежаться не может',
    }
  }
