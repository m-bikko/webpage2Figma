...
export type SceneText = {
  ...
  /** Как текстовому боксу подстраиваться под содержимое.
   *
   *  `auto-width` — строка одна, и бокс обязан вырасти по ней: Figma
   *  меряет шрифт своими метриками, и точная ширина браузера у неё
   *  оказывается на пиксель меньше нужного — тогда последний слог
   *  уезжает на вторую строку. Так «Recents» превратился в «Rece / nts»
   *  на живом импорте.
   *
   *  `fixed-width` — строк несколько; ширина берётся от содержимого
   *  элемента, то есть та, в которую переносил браузер, а высота
   *  отдаётся Figma. Переносы могут разойтись, но границы те же. */
  sizing: 'auto-width' | 'fixed-width'
  /** Боксы строк, снятые браузером, относительно текстового узла.
   *
   *  Применитель их НЕ использует: Figma переносит текст сама. Они
   *  едут ради кругового обхода — по ним эталонный рендерер рисует
   *  текст на том месте, где его видел браузер, и сверка ловит
   *  текстовый узел, поставленный не туда. Перенос строк остаётся
   *  единственным, что круговой обход о тексте не проверяет, и это
   *  записано в [[figma-semantics]]. */
...
export type SceneBase = {
  ...
  /** Узел создавал в CSS stacking context.
   *
   *  Нужен схлопыванию невидимых обёрток. Такой узел может не иметь ни
   *  заливки, ни рамки, ни прозрачности — `isolation: isolate` создаёт
   *  контекст сам по себе, — но убрать его нельзя: он ИЗОЛИРУЕТ
   *  смешение своих детей от того, что лежит под ним. Поймал это
   *  круговой обход на фикстуре `blend-isolated`, и поймал заслуженно:
   *  без поля признак терялся при построении сцены. */
  isolates: boolean
  children: SceneNode[]
}
