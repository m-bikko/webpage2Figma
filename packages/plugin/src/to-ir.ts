...
const nodeFrom = (
  ...
  /** Вектор проходит НАСКВОЗЬ, и круговой обход про него почти ничего
   *  не утверждает — это надо назвать прямо, а не выдавать за проверку.
   *
   *  Разбора у нас нет: SVG едет строкой от захвата до Figma, и здесь
   *  та же строка возвращается обратно. Совпадение поэтому
   *  тавтологично. Проверяют вектор два других места, и оба настоящие:
   *  ПИКСЕЛЬНЫЙ ГЕЙТ — что захваченный SVG рисует ровно то же, что
   *  рисовал браузер (три слома, слабейший в 18 раз больше бюджета), а
   *  УТВЕРЖДЕНИЯ О ФОРМЕ в `figma-shapes.ts` плюс сверка размера в
   *  применителе — что Figma разобрала его так, как мы ожидали.
   *
   *  Что круговой обход здесь всё-таки проверяет — геометрию и стиль
   *  БОКСА вокруг вектора: положение, размер, поворот, обёртку с
   *  фоном. Это не тавтология, потому что их строитель считает. */
  if (node.kind === 'vector') {
    return {
      ...common, kind: 'vector',
      style: { ...styleFrom(base), clip: false },
